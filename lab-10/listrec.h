...
template < class LE >
class List ;			//Prototype used by ListNode decalration
...
class List
{
  public:
    ...
    int full(void) const;
    int gotoBeginning(void);
    int gotoEnd(void);
    int gotoNext (void);
    LE getCursor(void) const;
  ...
  private:
    ...
    void clearSub ( ListNode<LE> *&p);
    void gotoEndSub ( ListNode<LE> *&p);
  ...
  private:
    ...
    int maxsize, counter;   // To control the maximum size of list
};
